...
from ctapipe.calib.camera.pedestals import PedestalFactory
...
class PedestalHDF5Writer(Tool):
    ...
    def start(self):
        ...
        # initialize the pedestal  containers
        self.container.pedestal.tels_with_data.append(self.pedestal.tel_id)

        for count, event in enumerate(self.eventsource):

            ped_data = self.pedestal.calculate_relative_gain(event)

            if ped_data:
                self.container.pedestal.tel[self.pedestal.tel_id] = ped_data
